...
c.currentX!==k&&c.update();return false});this.opts.init!==undefined&&this.opts.init(this)};e.prototype.update=function(a){a=jQuery.extend({animate:true,triggerMove:true},a);var b=this,f=function(){b.element.trigger("lectric.animationEnd");$(this).dequeue()};a.animate?this.element.animate({"margin-left":this.currentX+"px"}).queue(f):this.element.css({"margin-left":this.currentX+"px"}).queue(f);a.triggerMove&&this.element.trigger("lectric.move")};e.prototype.subscribe=function(a,b){var f=this;return this.element.bind("lectric."+
...
e.prototype.nearestPageX=function(a){return Math.round(a/this.structure.itemWidth())*this.structure.itemWidth()};e.prototype.pageX=function(a){return(this.opts.reverse?1:-1)*a*this.structure.itemWidth()};e.prototype.to=function(a){var b=self.currentX;this.currentX=this.limitXBounds(this.pageX(a));this.currentX!==b&&this.update();return false};e.prototype.nextPageX=function(a){if(this.page(a)+1<=this.structure.itemCount()-1)a-=this.structure.itemWidth();return a};e.prototype.previousPageX=function(a){if(this.page(a)>=
0)a+=this.structure.itemWidth();return a};e.prototype.limitXBounds=function(a){var b=this.structure.itemWidth()*this.structure.itemCount();if(this.opts.reverse){a=a>b-this.structure.itemWidth()?b-this.structure.itemWidth():a;a=a<0?0:a}else{a=a<-b+this.structure.itemWidth()?-b+this.structure.itemWidth():a;a=a>0?0:a}if(this.currentX-a>0&&this.opts.limitRight||this.currentX-a<0&&this.opts.limitLeft)a=this.currentX;return a};var d=function(){};d.prototype=new e;d.superobject=e.prototype;d.prototype.init=
function(a,b,f){d.superobject.init.call(this,a,b,f);this.element.parent().addClass("lectric-slider-touch");this.gesturing=false;a=$(a);a[0].addEventListener("touchstart",this,false);a[0].addEventListener("webkitTransitionEnd",this,false)};d.prototype.update=function(a){a=jQuery.extend({animate:true,triggerMove:true},a);a.animate&&this.decayOn();this.element.css({"-webkit-transform":"translate3d("+this.currentX+"px, 0, 0)"});a.triggerMove&&this.element.trigger("lectric.move")};d.prototype.handleEvent=
function(a){this[a.type](a)};d.prototype.click=function(a){this.moved&&a.preventDefault();this.element[0].removeEventListener("click",this,false);return false};d.prototype.touchstart=function(a){this.currentTarget=a.currentTarget;this.startX=a.touches[0].pageX-this.currentX;this.startY=a.touches[0].pageY-this.currentY;this.moved=false;g.addEventListener("gesturestart",this,false);g.addEventListener("gestureend",this,false);g.addEventListener("touchmove",this,false);g.addEventListener("touchend",this,
false);this.element[0].addEventListener("click",this,false);this.decayOff();this.element.trigger("lectric.start")};d.prototype.touchmove=function(a){if(this.gesturing)return false;if(!this.moved){Math.abs(a.touches[0].pageY-this.startY)<15&&a.preventDefault();this.element.trigger("lectric.firstMove")}this.moved=true;this.lastX=this.currentX;this.lastMoveTime=new Date;this.currentX=this.limitXBounds(a.touches[0].pageX-this.startX);this.update()};d.prototype.touchend=function(){g.removeEventListener("gesturestart",
this,false);g.removeEventListener("gestureend",this,false);g.removeEventListener("touchmove",this,false);g.removeEventListener("touchend",this,false);if(this.moved){this.currentX=this.nearestPageX(this.limitXBounds(this.currentX+(this.currentX-this.lastX)*100/(new Date-this.lastMoveTime+1)));this.update();this.element.trigger("lectric.end")}else this.element.trigger("lectric.endNoMove");this.currentTarget=undefined};d.prototype.webkitTransitionEnd=function(){this.element.trigger("lectric.animationEnd")};
d.prototype.gesturestart=function(){this.gesturing=true};d.prototype.gestureend=function(){this.gesturing=false};d.prototype.decayOff=function(){this.element.css({"-webkit-transition-duration":"0s"});this.element.css({"-webkit-transition-property":"none"})};d.prototype.decayOn=function(){this.element.css({"-webkit-transition-duration":"0.4s"});this.element.css({"-webkit-transition-property":"-webkit-transform"})};i.Slider=function(){return h.supportsTouch&&h.isWebkit?new d:new e};i.BaseSlider=e;i.TouchSlider=
d;g.Lectric=i})(window);
